...
const codePart1 = `import { createServerFn } from "@tanstack/react-start";
import { z } from "zod";
...
const part2 = `
    const recipientEmail = student.guardian_email?.trim();
    ...
    const htmlBody = (
      ...
      '<tr><td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Student Name:</td><td style="padding: 8px; border: 1px solid #cbd5e1;">' + student.full_name + '</td></tr>' +
      '<tr><td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Class:</td><td style="padding: 8px; border: 1px solid #cbd5e1;">' + className + '</td></tr>' +
      '<tr><td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold;">Term Average:</td><td style="padding: 8px; border: 1px solid #cbd5e1; font-weight: bold; color: #1e3a8a;">' + (reportCard.average ?? "N/A") + '%</td></tr>' +
      '</table>' +
      (reportCard.head_comment ? '<div style="background: #eef2ff; padding: 12px; border-left: 4px solid #4f46e5; margin-bottom: 20px;"><strong>Head Teacher\'s Remark:</strong> ' + reportCard.head_comment + '</div>' : '') +
...
fs.writeFileSync('src/lib/report-email.functions.ts', part1 + part2);
console.log('Successfully written report-email.functions.ts');
`;

fs.writeFileSync('make_fn.js', part1 + part2);
console.log('make_fn.js completed');
...
fs.writeFileSync('src/lib/report-email.functions.ts', codePart1 + codePart2);
console.log('report-email.functions.ts written successfully!');
